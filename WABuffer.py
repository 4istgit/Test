...
arcpy.management.CreateFeatureclass(out_gdb, os.path.basename(np_half_fc), "POLYLINE", spatial_reference=target_sr)
...
skipped_np = 0
with arcpy.da.InsertCursor(np_half_fc, fields_insert) as icur, arcpy.da.SearchCursor(np_lines_single_fc, fields_search) as scur:
...
# ---------------------------------------------------------------------
# Get intersection vertices from Np lines
...
arcpy.management.CreateFeatureclass(scratch_gdb, os.path.basename(np_x_pts_true_fc), "POINT", spatial_reference=target_sr)
if "PT_ID" not in [f.name for f in arcpy.ListFields(np_x_pts_true_fc)]:
    arcpy.management.AddField(np_x_pts_true_fc, "PT_ID", "LONG")
kept = 0
checked = 0
with arcpy.da.InsertCursor(np_x_pts_true_fc, ["PT_ID", "SHAPE@"]) as ic_true, \
     arcpy.da.SearchCursor(np_x_pts_raw_fc, ["OID@", "SHAPE@"]) as sc_pts:
    for pt_oid, pt_geom in sc_pts:
        checked += 1
        arcpy.management.SelectLayerByLocation(np_lines_layer, "INTERSECT", pt_geom, selection_type="NEW_SELECTION")
...
if export_final_shp:
    try:
        ...
        keep_fields = ["BUF_FT", bankfull_field]  # add other must-keep fields
